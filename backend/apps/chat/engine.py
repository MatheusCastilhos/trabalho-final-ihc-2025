...
class ChatEngine:
    ...
    - Montar o contexto (histórico + system).
    ...
    def __init__(
        ...
        # Carrega variáveis do .env (procura .env a partir do diretório atual e pais)
        load_dotenv()
        ...
        # Histórico interno (se tu quiser usar em modo terminal depois)
    ...
    def reset_history(self) -> None:
        """Limpa o histórico interno da conversa (não mexe no system prompt)."""
        self.history.clear()
    ...
    def _build_messages(self, new_user_message: str, history: List[Message]) -> List[Message]:
        ...
        `history` aqui é o histórico vindo do banco (user/assistant).
        """
        messages: List[Message] = [
            {"role": "system", "content": self.system_prompt}
        ]

        # Histórico salvo no banco (user/assistant)
    ...
    def send_message(self, user_input: str, history: List[Message]) -> str:
        """
        Recebe o texto do usuário e o histórico vindo do banco,
        envia para o modelo e devolve a resposta.
        NÃO atualiza o self.history interno — isso fica a cargo
        da camada de persistência (models/views).
        """
